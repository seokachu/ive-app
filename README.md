...
- **오프라인 · 로드 실패 화면** — 연결이 끊기거나(NetInfo) 페이지 로드에 실패하면(`onError`)
  WebView 내장 에러 페이지 대신 브랜드 화면을 띄우고(그라데이션 아이콘 + 다시 시도/홈으로 +
  Wi-Fi·데이터·비행기 모드 점검 칩), 연결이 복구되면 실패한 페이지를 자동으로 다시 불러온다
- Android 당겨서 새로고침, iOS WebView 프로세스 종료 자동 복구
